...
{
    private readonly RecordingPageViewModel _owner; private bool _renaming; private string _editName;
    ...
    public bool IsRenaming { get=>_renaming; set=>SetProperty(ref _renaming,value); }
    public string EditName { get=>_editName; set=>SetProperty(ref _editName,value); }
    public ICommand SelectCommand{get;} public ICommand CancelSelectCommand{get;} public ICommand StartRenameCommand{get;} public ICommand ConfirmRenameCommand{get;} public ICommand CancelRenameCommand{get;} public ICommand DeleteCommand{get;} public ICommand OptimizeCommand{get;} public ICommand SetHotkeyCommand{get;}
    public RecordingItemViewModel(Recording model,RecordingPageViewModel owner){Model=model;_owner=owner;_editName=model.Name;
        ...
        StartRenameCommand=new RelayCommand(_=>{EditName=Model.Name;IsRenaming=true;},_=>owner.IsIdle);ConfirmRenameCommand=new RelayCommand(_=>Confirm());CancelRenameCommand=new RelayCommand(_=>{EditName=Model.Name;IsRenaming=false;});
    ...
    public void Confirm(){var n=EditName.Trim();if(n.Length==0)return;Model.Name=n;IsRenaming=false;_owner.Save(Model);OnPropertyChanged(nameof(Name));}
    public void Refresh(){OnPropertyChanged(nameof(IsSelected));OnPropertyChanged(nameof(DurationText));OnPropertyChanged(nameof(ActionCountText));OnPropertyChanged(nameof(HotkeyText));}
}
...
public sealed class RecordingPageViewModel : ViewModelBase
{
    ...
    private RecordingItemViewModel? _selected; private RecordingState _state; private string _search="",_elapsed="00:00:00"; private int _actionCount; private double _speed=1; private int _loop=1,_interval;
    public ObservableCollection<RecordingItemViewModel> Recordings{get;}=new(); public ICollectionView FilteredRecordings{get;}
    ...
    public string SearchText{get=>_search;set{if(SetProperty(ref _search,value))FilteredRecordings.Refresh();}} public string ElapsedTime{get=>_elapsed;private set=>SetProperty(ref _elapsed,value);} public int ActionCount{get=>_actionCount;private set=>SetProperty(ref _actionCount,value);}
    ...
    public event Action<RecordingItemViewModel>? OptimizeRequested; public event Action<RecordingItemViewModel>? HotkeyRequested; public event Action<string>? RecordingDeleted;
    public RecordingPageViewModel(RecordingStorageService storage,GlobalHookService hook,PlaybackService playback){_storage=storage;_hook=hook;_playback=playback;
        foreach(var r in storage.LoadAll())Recordings.Add(new(r,this));FilteredRecordings=CollectionViewSource.GetDefaultView(Recordings);FilteredRecordings.Filter=o=>string.IsNullOrWhiteSpace(SearchText)||((RecordingItemViewModel)o).Name.Contains(SearchText,StringComparison.CurrentCultureIgnoreCase);
    ...
    internal void RequestOptimize(RecordingItemViewModel i)=>OptimizeRequested?.Invoke(i); internal void RequestHotkey(RecordingItemViewModel i)=>HotkeyRequested?.Invoke(i);
